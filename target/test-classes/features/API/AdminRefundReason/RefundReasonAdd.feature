Feature: As an administrator, I want to be able to add a new Refund&Reason record to the system via the API connection.
  ...
  Scenario: When a POST body containing valid authorization information and necessary data (reason) is sent to the
  ...
  response body should be verified as "refundReason added successfully".
    ...
    * The api user prepares a POST request containing the necessary information to send to the api refundReasonAdd endpoint.
    # Api kullanicisi api refundReasonAdd endpointine gondermek icin gerekli bilgileri iceren bir post request hazirlar
  ...
  with the added item id returned in the response body.)
  ...
  Scenario: When a POST body containing invalid authorization information and necessary data (reason) is sent to the
