Feature: As an administrator, I want to be able to delete the record specified by the FAQ's id via the API connection.
  ...
  Scenario: When a DELETE request containing valid authorization credentials and the FAQ's id to be deleted is sent to the
  ...
  body should confirm: "faqs deleted successfully".
    ...
    * The api user prepares a DELETE request containing the FAQ ids to be deleted to send to the api faqsDelete endpoint.
  ...
  message in the response body should confirm: "faqs not found".
    ...
    * The api user sets "api/faqsDelete" path parameters
    # APi kullanicisi "api/faqsDelete" path parametrelerini olusturur
    * The api user prepares a DELETE request containing the FAQ ids that are not present in the system to send to the api faqsDelete endpoint.
  ...
  Scenario: When a DELETE request containing invalid authorization credentials and the FAQ's id to be deleted is sent to
