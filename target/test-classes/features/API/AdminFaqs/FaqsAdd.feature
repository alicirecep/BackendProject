Feature: As an administrator, I want to be able to create a new FAQ record via API connection
  ...
  Scenario Outline: When a POST request containing valid authorization credentials and necessary information (title, description) is
  ...
  response body should confirm: "Faqs added successfully".
    ...
    * The api user prepares a POST request containing the "<title>" and "<description>" information to send to the api faqsAdd endpoint.
    # Api kullanicisi api faqsAdd endpointine gondermek icin "<title>" ve "<description>" bilgilerini iceren bir post request hazirlar
    ...
    * The api user verifies that the message information in the response body is "Faqs added successfully"
    ...
    Examples:
      | title                   | description                                                                                                                                                |
      | How does the site work? | Select your items, add them to your cart, provide your address and choose the delivery time, then complete the order. There you go, your shopping is done! |
  ...
  Scenario Outline: When a POST request containing invalid authorization credentials and necessary information (title, description)
