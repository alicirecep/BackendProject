Feature: As an administrator, I want to be able to update the information of the FAQ record with the specified FAQ id via API connection.
  ...
  Scenario Outline: When a PATCH request containing valid authorization credentials, the FAQ id to be updated, and the updated
  ...
  should be 202, and the message in the response body should confirm: "faqs updated successfully".
    ...
    * The api user prepares a PATCH request containing the "<title>" data to send to the api faqsUpdate endpoint.
    # Api kullanicisi api faqsUpdate endpointine gondermek icin "<title>" verisini iceren bir patch request hazirlar
  ...
  returned should be 202, and the message in the response body should confirm: "faqs updated successfully".
    ...
    * The api user verifies that the message information in the response body is "faqs updated successfully"
    ...
    Examples:
      ...
      | 25 | How do I know when my order is here? | A representative will call you as soon as they are at your house to let you know about your delivery. |
  ...
  confirm: "faqs not found".
    ...
    * The api user saves the response returned from the api faqsUpdate endpoint and confirms that the status code is '404' and the reason phrase is Not Found.
    ...
    Examples:
      | id  | title                                | description                                                                                           |
      | 187 | How do I know when my order is here? | A representative will call you as soon as they are at your house to let you know about your delivery. |


  Scenario Outline: When a PATCH request containing invalid authorization credentials, the FAQ id to be updated, and the updated
  information of the FAQ (title, description) is sent to the /api/faqsUpdate/{id} endpoint, the expected status code
  ...
  Scenario Outline: The updated_Id information in the response body from the /api/faqsUpdate/{id} endpoint should be verified to be
  ...
  updated_Id returned in the response body.)
    ...
    * The api user verifies that the title information in the response body is "<titleValue>".
    ...
    Examples:
      | id | titleValue                           |
      | 16 | How do I know when my order is here? |
      | 25 | How do I know when my order is here? |
