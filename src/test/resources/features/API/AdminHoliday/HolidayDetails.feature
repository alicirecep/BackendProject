Feature: As an administrator, I should be able to access detailed information about the holiday with the
  ...
  type, date, created_at, updated_at) should be validated.
    ...
    * The api user verifies the content of the data <id>, "<year>", "<name>", <type>, "<date>", "<created_at>", "<updated_at>" in the response body.
    ...
    Examples:
      ...
      | 18 | 18 | 2024 | Ramadan Holiday | 1    | 2024-04-10,2024-04-12 | 2024-03-23T19:26:18.000000Z | 2024-03-23T19:26:18.000000Z |
  ...
  message in the response body should confirm: "holiday not found".
    ...
    * The api user constructs the base url with the "admin" token.
    # APi kullanicisi "admin" token ile base urli olusturur
    * The api user sets "api/holidayDetails" path parameters
    * The api user prepares a GET request containing the holiday <id> that are not present in the system to send to the api holidayDetails endpoint.
  ...
  Scenario Outline: When a GET request containing invalid authorization credentials and the holiday id data for the desired detailed
