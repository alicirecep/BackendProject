Feature: As an administrator, I want to be able to update the Refund&Reason information with the specified ID number via the
  ...
  Scenario: When a PATCH request body containing valid authorization information, the Refund&Reason ID to be updated,
  ...
  Scenario: When a PATCH request body containing valid authorization information and an incorrect (non-existent in the
  ...
  verified as "refundReason not found".
    ...
    * The api user constructs the base url with the "admin" token.
    # APi kullanicisi "admin" token ile base urli olusturur
    * The api user sets "api/refundReasonUpdate/425" path parameters
    # APi kullanicisi "api/refundReasonUpdate/{id}" path parametrelerini olusturur
    * The api user prepares a PATCH request containing the necessary data to send to the api refundReasonUpdate endpoint.
    # Api kullanicisi api refundReasonUpdate endpointine gondermek icin gerekli veriyi iceren bir patch request hazirlar
  ...
  Scenario: When a PATCH request body containing invalid authorization information, along with the Refund&Reason ID to
